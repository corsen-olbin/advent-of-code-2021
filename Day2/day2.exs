defmodule AOCDay2 do
  ...
  def calculate_depth_and_distance(list) do
    calculate_depth_and_distance_rec(list, {0, 0})
  end
  ...
  def calc_new_depth_and_distance({"forward", num}, {horizontal, depth}), do: {horizontal + num, depth}
  def calc_new_depth_and_distance({"down", num}, {horizontal, depth}), do: {horizontal, depth + num}
  def calc_new_depth_and_distance({"up", num}, {horizontal, depth}), do: {horizontal, depth - num}

  def print_answer({horizontal, depth}), do: IO.puts("Product of horizontal distance and depth: #{horizontal * depth}")
end
