defmodule AOCDay11 do
  ...
  def count_octopus_flashes(body) do
    body
    |> String.split()
    |> turn_grid_into_map()
    |> game_loop_count_flashes(0, 100)
    |> print_answer
  end
  ...
  # game loop
  # if 0 days left, return flashes
  def game_loop_count_flashes(_, flashes, 0), do: flashes
  # else, calculate next state and running flashes, decrement day
  def game_loop_count_flashes(state, flashes, days_left) do
    #IO.inspect(state, label: " day #{10 - days_left} flashes #{flashes}")

    {flashed_state, new_flashes} = calc_new_state(state, 0)
    new_state = clear_flashes(flashed_state)

    game_loop_count_flashes(new_state, new_flashes + flashes, days_left - 1)
  end

  def clear_flashes(state) do
    Map.new(state, fn {k, inner} -> {k, Map.new(inner, fn {ik, {_, x}} -> {ik, {:no_flash, x}} end)} end)
  end
  ...
  def print_answer(answer), do: IO.puts("Num of octopus flashes: #{answer}")
end

case File.read("./day11values.txt") do
  {:ok, body} -> AOCDay11.count_octopus_flashes(body)
