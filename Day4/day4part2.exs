defmodule AOCDay4 do
  ...
  def find_last_winning_bingo_card(input) do
  ...
  def loop_game_until_win(nums_called, [next_num | future_nums], boards) do
    ...
    winning_cards =
      Enum.filter(boards, fn x -> is_winning_card?(new_nums_called, x) end)

    unwon_boards = Enum.reduce(winning_cards, boards, fn x, acc -> List.delete(acc, x) end)

    case Enum.count(unwon_boards) do
      0 -> {new_nums_called, List.first(winning_cards)}
      _ -> loop_game_until_win(new_nums_called, future_nums, unwon_boards)
    end
  end
...
case File.read("./day4values.txt") do
  {:ok, body} -> AOCDay4.find_last_winning_bingo_card(body)
