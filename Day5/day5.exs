defmodule AOCDay5 do
  def find_safe_places(body) do
    ...
    filtered_paths =
      Enum.filter(paths, fn %{from: {x1, y1}, to: {x2, y2}} -> x1 == x2 || y1 == y2 end)

    IO.inspect(filtered_paths, label: "filtered")
    heat_map = create_map(filtered_paths)
  ...
  def convert_path_to_line(%{from: {x1, y1}, to: {x2, y2}}) do
    {xs, ys} =
      cond do
        ...
        true ->
          raise "not horizontal or vertical line"
      end
